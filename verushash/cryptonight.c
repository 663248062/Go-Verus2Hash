...
uint64_t mul128(uint64_t a, uint64_t b, uint64_t* product_hi)
{
...
/*static inline uint64_t mul128(uint64_t a, uint64_t b, uint64_t* product_hi)
{
	...
	__asm__("mul %%rdx":"=a" (lo), "=d" (hi):"a" (a), "d" (b));
...
static inline int monero_variant() {
	//return (work->is_monero && work->data[0] >= 7) ? work->data[0] - 6 : 0;
	return 1;
}
//0707999de9d705d9665e876ca90c2eb568732693283b481a8c07934ac09c8bfd9292ece8754d75a42900005fe787fb2c8ceb3a4e17562c589fa6b1208bad7b45b48c22ca9976a163fbddab0700000000
void cryptonight_regenhash(void* input, void* state)
{
	//uint32_t data[20];
	int variant = monero_variant();

	cryptonight(state, input, 76, variant);
}
