...
extern "C" u128 _mm_setr_epi8_emu(u_char c0, u_char c1, u_char c2, u_char c3, u_char c4, u_char c5, u_char c6, u_char c7, u_char c8, u_char c9, u_char c10, u_char c11, u_char c12, u_char c13, u_char c14, u_char c15)
{
    __m128i result;
    ((uint8_t *)&result)[0] = c0;
    ((uint8_t *)&result)[1] = c1;
...
extern "C" __m128i _mm_load_si128_emu(const void *p)
{
    return *(__m128i *)p;
}

extern "C" void _mm_store_si128_emu(void* p, __m128i val)
{
    *(__m128i *)p = val;
}
...
extern "C" void print_bin(uint8_t * cmd, size_t len);
...
extern "C" uint64_t verusclhash_port(void * random, const unsigned char buf[64], uint64_t keyMask) {
	char * ret = new char[8];
	...
	uint64_t a= precompReduction64_port(acc);
	return a;
}
